...
def run_miniprot_and_fix(workflow4_dir):
    """运行miniprot并修复GFF"""
    # 1. 动态获取.fna文件（优先找用户输入的ninanjie1.fna，找不到则用目录下第一个.fna文件）
    fna_files = glob.glob(os.path.join(workflow4_dir, "*.fna")) + \
                glob.glob(os.path.join(workflow4_dir, "*.fa")) + \
                glob.glob(os.path.join(workflow4_dir, "*.fasta"))
    
    if not fna_files:
        print(f"错误: 在 {workflow4_dir} 中未找到任何.fna/.fa/.fasta文件")
        return False
    
    genome_fasta = fna_files[0]  # 默认使用找到的第一个文件
    # 如果用户输入是ninanjie，则尝试匹配ninanjie1.fna
    input_name = os.path.basename(input_root)  # 从输入路径获取名称
    if input_name:
        target_fna = os.path.join(workflow4_dir, f"{input_name}1.fna")
        if os.path.exists(target_fna):
            genome_fasta = target_fna
